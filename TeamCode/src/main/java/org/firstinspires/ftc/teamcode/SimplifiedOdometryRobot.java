...
public class SimplifiedOdometryRobot {
    ...
    private final double ODOM_INCHES_PER_COUNT   = 0.00052843824; //0.002969;   //  GoBilda Odometry Pod (1/226.8); 35mm wheel and 8192 ticks; 109.95574 circumference in mm; 4.32896614173 in inches; divided by 8192;
    ...
    public final double DRIVE_GAIN          = 0.07;//0.75; //.037    // Strength of axial position control
    public final double DRIVE_ACCEL         = 2.0;     // Acceleration limit.  Percent Power change per second.  1.0 = 0-100% power in 1 sec.
    public final double DRIVE_TOLERANCE     = 2.0;//1.0;     // Controller is is "inPosition" if position error is < +/- this amount
    public final double DRIVE_DEADBAND      = 0.2;     // Error less than this causes zero output.  Must be smaller than DRIVE_TOLERANCE
    public double DRIVE_MAX_AUTO      = 1.0;//0.6;     // "default" Maximum Axial power limit during autonomous

    public final double STRAFE_GAIN         = 0.12; //.061    // Strength ovf   f lateral position control
    public final double STRAFE_ACCEL        = 1.5; //1    // Acceleration limit.  Percent Power change per second.  1.0 = 0-100% power in 1 sec.
    public final double STRAFE_TOLERANCE    = 2.0;//0.75; //.5    // Controller is is "inPosition" if position error is < +/- this amount
    public final double STRAFE_DEADBAND     = 0.2; //.2    // Error less than this causes zero output.  Must be smaller than DRIVE_TOLERANCE
    public double STRAFE_MAX_AUTO     = .6; //1    // "default" Maximum Lateral power limit during autonomous

    public double YAW_GAIN            = 0.033;//0.036;//0.018;    // Strength of Yaw position control
    public double YAW_ACCEL           = 3.0;     // Acceleration limit.  Percent Power change per second.  1.0 = 0-100% power in 1 sec.
    public double YAW_TOLERANCE       = 2.0;//1.0;     // Controller is is "inPosition" if position error is < +/- this amount
    public double YAW_DEADBAND        = 0.25;    // Error less than this causes zero output.  Must be smaller than DRIVE_TOLERANCE
    public double YAW_MAX_AUTO        = 0.6;     // "default" Maximum Yaw power limit during autonomous
    ...
    public void initialize(boolean showTelemetry)
    {
        ...
        driveEncoder = myOpMode.hardwareMap.get(DcMotor.class, "ascentMotor2"); // axial
        strafeEncoder = myOpMode.hardwareMap.get(DcMotor.class, "ascentMotor1"); // lateral
    ...
    public void driveWithTimeout(double distanceInches, double power, double holdTime, double timeoutSec) {
        resetOdometry();
        ...
        while (myOpMode.opModeIsActive() && readSensors()) {
            myOpMode.telemetry.addData("timeout", timeout.seconds());
            // implement desired axis powers
            moveRobot(driveController.getOutput(driveDistance), strafeController.getOutput(strafeDistance), yawController.getOutput(heading));

            // Time to exit?
            if ((driveController.inPosition() && yawController.inPosition()) || (timeout.seconds() > timeoutSec)) {
                if (holdTimer.time() > holdTime) {
                    break;   // Exit loop if we are in position, and have been there long enough.
                }
            } else {
    ...
    public void strafeWithTimeout(double distanceInches, double power, double holdTime, double timeoutSec) {
        resetOdometry();
        ...
        while (myOpMode.opModeIsActive() && readSensors()){
            ...
            // Time to exit?
            if (strafeController.inPosition() && yawController.inPosition() || (timeout.seconds() > timeoutSec)) {
                if (holdTimer.time() > holdTime) {
                    break;   // Exit loop if we are in position, and have been there long enough.
                }
            } else {
    ...
    public void strafeWithCamera(double distanceInches, double power, double holdTime, PredominantColorProcessor.Swatch color) {
        resetOdometry();
        ...
        while (myOpMode.opModeIsActive() && readSensors()){
            ...
            // Time to exit?
            if ((strafeController.inPosition() && yawController.inPosition()) || (PredominantColorProcessor.Swatch.BLUE == result.closestSwatch) || (PredominantColorProcessor.Swatch.RED == result.closestSwatch) || (PredominantColorProcessor.Swatch.YELLOW == result.closestSwatch)) {
                if (holdTimer.time() > holdTime) {
                    break;   // Exit loop if we are in position, and have been there long enough.
                }
            } else {
    ...
    public void turnToWithTimeout(double headingDeg, double power, double holdTime, double timeoutSec) {
        ...
        while (myOpMode.opModeIsActive() && readSensors()) {
            ...
            // Time to exit?
            if (yawController.inPosition() || (timeout.seconds() > timeoutSec)) {
                if (holdTimer.time() > holdTime) {
                    break;   // Exit loop if we are in position, and have been there long enough.
                }
            } else {
     ...
     * @param holdTime Minimum time (sec) required to hold the final position.  0 = no hold.
     */
    public void turnToWithCamera(double headingDeg, double power, double holdTime, PredominantColorProcessor.Swatch color) {
        ...
        while (myOpMode.opModeIsActive() && readSensors()) {
            ...
            // Time to exit?
            if (yawController.inPosition() || (PredominantColorProcessor.Swatch.BLUE == color) || (PredominantColorProcessor.Swatch.RED == color) || (PredominantColorProcessor.Swatch.YELLOW == color)) {
                if (holdTimer.time() > holdTime) {
                    break;   // Exit loop if we are in position, and have been there long enough.
                }
            } else {
